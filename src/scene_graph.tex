...
\begin{figure}
	\begin{subfigure}[t]{.5\textwidth}
		\centering
		\includegraphics[width=\textwidth]{scene_graph/psgtr_output.png}
		\caption{Risultati di Segmentazione }
	\end{subfigure}
	\begin{subfigure}[t]{.5\textwidth}
		\centering
		\begin{tikzpicture}[ >={Stealth[round]},  % Arrow tip
			auto,
			...
			\begin{scope}[node distance=1.2cm and 0.62cm]
				\node (palla da tennis) [circle, draw, scale=0.7, align=center] at (2, 1.5) {sports\\ball};
				\node (bottiglia) [circle, draw, scale=0.7, align=center] at (-2, 3) {bottle};
				\node (zoccolino muro) [circle, draw, scale=0.7, align=center] at (0, 2) {wall-\\tile};
				\node (libro) [circle, draw, scale=0.7, align=center] at (-2, 1) {book};
				\node (pavimento) [circle, draw, scale=0.7, align=center] at (0, 0) {floor-\\other-\\merged};
			\end{scope}

			% Define edges
			\draw[<->] (bottiglia) to [bend left=30] node[midway, right] {\tiny{beside}} (palla da tennis);
			\draw[<->] (zoccolino muro) to node[pos=0.2, left] {\tiny{attached to}} (pavimento);
			\draw[->] (libro) to [bend right=30] node[midway, left] {\tiny{on}} (pavimento);
			\draw[->] (libro) to [bend right=30] node[midway, right] {\tiny{under}} (bottiglia);
			\draw[->] (bottiglia) to [bend right=30] node[midway, left] {\tiny{on}} (libro);
			\draw[->] (pavimento) to [bend right=30] node[pos=0.1, left] {\tiny{under}} (libro);
			\draw[->] (pavimento) to [bend right=30] node[pos=0.3, right] {\tiny{under}} (palla da tennis);
			\draw[->] (palla da tennis) to [bend right=30] node[pos=0.4, left] {\tiny{on}} (pavimento);
		\end{tikzpicture}
		\caption{Risultati del Grafo di Scena}
	\end{subfigure}
	\caption{}
\end{figure}
...
Il paper fornisce i modelli PSGFormer e PSGTr in diverse configurazioni: Backbone ResNet-50 e ResNet-101, e modelli addestrati in 12 e 60 epoche. Per il nostro caso d'uso abbiamo scelto PSGTr in versione \textbf{ResNet-50 addestrato in 60 epoche} poiché è quello che ha ottenuto i risultati migliori per la generazione del grafo di scena, considerandosi Stato dell'Arte.\\\\
\begin{table}[H]
	\centering
	\resizebox{\textwidth}{!}{
		\begin{tabular}{llcccccc}
			\toprule
			Backbone & Method     & \multicolumn{3}{c}{Predicate Classification} & \multicolumn{3}{c}{Scene Graph Generation}                                                                                             \\
			\cmidrule(lr){3-5} \cmidrule(lr){6-8}
			         &            & R/mR@20                                      & R/mR@50                                    & R/mR@100             & R/mR@20              & R/mR@50              & R/mR@100             \\
			\midrule
			\multirow{6}{*}{ResNet-50}
			         & IMP        & 31.9 / 9.55                                  & 36.8 / 10.9                                & 38.9 / 11.6          & 16.5 / 6.52          & 18.2 / 7.05          & 18.6 / 7.23          \\
			         & MOTIFS     & 44.9 / 20.2                                  & 50.4 / 22.1                                & 52.4 / 22.9          & 20.0 / 9.10          & 21.7 / 9.57          & 22.0 / 9.69          \\
			         & VCTree     & \textbf{45.3 / 20.5}                         & \textbf{50.8 / 22.6}                       & \textbf{52.7 / 23.3} & \textbf{20.6 / 9.70} & \textbf{22.1 / 10.2} & \textbf{22.5 / 10.2} \\
			         & GPSNet     & 31.5 / 13.2                                  & 39.9 / 16.4                                & 44.7 / 17.3          & 16.9 / 7.49          & 19.6 / 7.49          & 20.7 / 7.63          \\
...
\begin{figure}[H]
	...
	\begin{tikzpicture}
		\begin{axis}[
				ybar,
				symbolic x coords={PSGTr+, PSGFormer+, Mask DINO},
				xtick=data,
				ylabel={PQ},
				ymin=0, ymax=100,
				bar width=15pt,
			...
			\addplot[ybar, fill=orange] coordinates {(PSGTr+, 13.9)};
			\addplot[ybar, fill=green] coordinates {(PSGFormer+, 36.8)};
			\addplot[ybar, fill=teal] coordinates {(Mask DINO, 59.4)};
		\end{axis}
	\end{tikzpicture}
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\If{$distance > min\_distance$ \textbf{and} $distance < max\_distance$ \textbf{and} $obj\_classes[i]$ \textbf{not in} $blacklist$}
...
\begin{figure}[H]
	\begin{subfigure}[t]{0.45\textwidth}
		\centering
		\includegraphics*[width=\textwidth]{commons/scene_table.png}
	\end{subfigure}
	\begin{subfigure}[t]{0.55\textwidth}
		...
		\begin{tikzpicture}[
			...
			\begin{scope}[node distance=1.2cm and 0.62cm]
				\node (i) [circle, draw, scale=0.7, above=of a] {Floor};
			\end{scope}
...
\begin{figure}[H]
	\begin{subfigure}[t]{0.45\textwidth}
		\centering
		\includegraphics*[width=\textwidth]{commons/scene_box.png}
	\end{subfigure}
	\begin{subfigure}[t]{0.55\textwidth}
		\centering
		\begin{tikzpicture}[
...
\begin{figure}[H]
	\begin{subfigure}[t]{0.45\textwidth}
		\centering
		\includegraphics*[width=\textwidth]{commons/scene_table_bottle.png}
	\end{subfigure}
	\begin{subfigure}[t]{0.55\textwidth}
		\centering
		\begin{tikzpicture}[
			...
			\begin{scope}[node distance=1cm and 0.62cm]
				\node (e) [circle, draw, scale=0.7, below=of a] {Chair};
				\node (d) [circle, draw, scale=0.7, left=of e] {Table};
				\node (f) [circle, draw, scale=0.7, right=of e] {Bottle};
			\end{scope}
